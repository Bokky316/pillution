...
persistor.subscribe(() => {
   ...
   createRoot(document.getElementById('root')).render(
       <Provider store={store}> {/* Redux Store를 React 앱에 적용하기 위한 설정 */}
           <PersistGate loading={null} persistor={persistor}> {/* Redux Persist로 상태를 복원하기 위한 설정 */}
               <BrowserRouter>
                   <App />
               </BrowserRouter> {/* React Router를 사용하여 클라이언트 사이드 라우팅을 활성화하기 위한 설정 */}
           </PersistGate>
       </Provider>
   )
