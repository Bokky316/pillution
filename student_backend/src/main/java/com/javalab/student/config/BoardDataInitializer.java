    ...
    private void initializePosts() {
        ...
        List<Post> posts = List.of(
                ...
                createPost(adminMember.get().getId(), faqBoard, "필루션 영양제와 다른 영양제를 함께 먹어도 될까요?",
                        "정확한 상담을 위해서는 드시고 계신 타사 영양제의 성분을 알기 위해 품명을 알아야 해요.  품명과 함께 필루션 상담원에게 문의하시면 빠르고 정확하게 답변을 드릴게요.", "제품"),

                createPost(adminMember.get().getId(), faqBoard, "결제 취소는 어떻게 하나요?",
                        "주문 상태가 배송 전으로 표시되는 경우 -> 결제일 당일 12시(정오) 이전 홈페이지 -> 마이페이지 -> 결제관리에서 언제든 주문 취소가 가능해요. 주문 상태가 배송 중 또는 배송 완료인 경우 -> 결제일 당일 12시(정오) 이후 필루션 상담원 채팅 상담을 통해 교환 및 반품 접수를 해주셔야 돼요. (위의 경우 및 배송 완료 후 취소의 경우에는 취소 수수료가 발생할 수 있어요) 주문 취소가 된 건은 결제 수단에 따라 영업일 기준으로 3일~7일 이내에 승인 취소 또는 환불 처리를 해드려요. (결제 취소 시에는 해당 결제를 통해 제공된 혜택이 공제된 후에 제공돼요)", "교환/반품"),

                createPost(adminMember.get().getId(), faqBoard, "배송 조회는 어떻게 하나요?",
                        "정기구독으로 구매하신 경우 홈페이지 -> 마이페이지 -> 결제관리 -> 해당 결제건 클릭 -> 운송장 번호 클릭 비회원으로 한 번만 구매하기를 하신 경우 홈페이지 -> 고객센터 -> 비회원 주문 -> 주문번호 또는 연락처 입력, 주문 확인 비밀번호 입력 -> 운송장 번호 클릭", "배송"),

                createPost(adminMember.get().getId(), faqBoard, "정기구독을 잠시 중단할 수 있나요?",
                        "정기구독 중단 기능은 없어요. 다만 최대 3개월까지 정기구독 결제일을 연기하실 수 있어요. 홈페이지 -> 마이페이지 -> 정기구독에서 결제일 변경 또는 상담원 채팅을 통해 변경하실 수 있어요. ※ 결제 2일 전에 결제 예정 알림을 드리니 안심하셔도 돼요.", "주문/결제"),

                createPost(adminMember.get().getId(), faqBoard, "정기구독 제품을 변경할 수 있나요?",
                        "정기구독 제품을 추가 또는 제거하실 수 있어요. 제품 추가 홈페이지-> 마이페이지 -> 정기구독 -> 상품정보 -> 수정하기 -> 제품 추가하기 -> 상품정보 저장하기 제품 제거 홈페이지 -> 마이페이지 -> 정기구독 -> 상품정보 -> 수정하기 -> 제품 우측 x 표시 클릭 -> 상품정보 저장하기", "주문/결제"),
                ...
                createPost(adminMember.get().getId(), faqBoard, "비밀번호가 생각나지 않아요.",
                        "필루션 홈페이지에서 회원가입하신 경우 홈페이지 -> 로그인 -> 비밀번호 찾기 SNS계정으로 가입하신 경우(네이버, 카카오 등) 가입하신 SNS를 통해 찾으셔야 돼요. 상담원을 통해 가입 아이디만 알려드릴 수 있어요.", "회원정보"),

                createPost(adminMember.get().getId(), faqBoard, "회원정보 변경은 어떻게 하나요?",
                        "가입하신 회원정보 변경을 원하시는 경우 홈페이지 -> 마이페이지 -> 회원관리에서 수정하실 수 있어요. 계정에 등록된 연락처 변경 혹은 아이디 변경은 상담원을 통해 요청하실 수 있어요.", "회원정보"),

                createPost(adminMember.get().getId(), faqBoard, "제품을 교환/반품하고 싶어요.",
                        "교환 및 반품의 경우 필루션 상담원을 통해 접수가 가능하며 아래의 교환 및 반품 규정에 따라 진행돼요. (필요 시 제품의 상태 이미지 첨부 필요) 수령한 제품에 문제가 있는 경우 제품이 표시 및 광고된 내용과 다르거나 제품 자체에 하자가 있는 경우에는 재배송 또는 환불 처리를 해드려요. 제품을 받으시고 제품의 문제를 확인 가능한 사진을 첨부해서 필루션 상담원에게 전송 부탁드려요. 제품에 문제가 있는 것으로 확인될 경우 모든 배송비는 필루션이 부담해요. 고객님의 변심 또는 주문 오류가 있는 경우 제품의 상태에 문제가 없는 경우에는 제품을 받으신 날부터 7일 이내에 교환 및 반품 신청이 가능해요. 단순 변심에 의한 교환 및 반품을 요청하신 경우 제품 발송을 위한 배송비와 반품을 위한 배송비가 부담돼요. (필루션 정기구독의 경우 배송비 무료로 제공이 되나 본 경우에는 발송 배송비와 반품 배송비를 포함하여 5,000원을 부담하셔야 해요) 제품에 따라 도서/산간지역의 경우 배송비가 추가될 수 있어요. 교환 및 반품 불가 안내 아래와 같은 사항에는 교환 및 반품이 어려울 수 있으니 양해 부탁드려요. - 고객님의 책임으로 제품이 훼손된 경우 (제품 박스의 스티커를 제거한 경우 포함) - 고객님의 사용으로 제품의 가치가 현저하게 감소한 경우", "교환/반품"),

                createPost(adminMember.get().getId(), faqBoard, "정기구독과 한 번만 구매하기의 차이가 무엇인가요?",
                        "정기구독이란? 매월(30일 기준) 영양제가 소진되기 전에 정기적으로 배송되는 서비스에요. 필루션은 고객님들의 건강을 지켜드리기 위해 꾸준히 영양성분을 섭취하는 것이 매우 중요하다고 생각하고 있어요. 그래서 따로 신경 쓰지 않아도 정기적으로 배송이 되는 배송 서비스를 제공해 드려요. 한 번만 구매하기란? 일회성으로 제품을 낱개로 구매하실 수 있어요.(다만, 배송비가 추가돼요)", "주문/결제")
        );
